...
#include "stdio.h"
#include "tlhelp32.h"
#include "io.h"     // 기능 : 파일을 액세스하기 전에 파일이 실제로 존재하는 지 확인하는 함수
#include "tchar.h"
...
BOOL InjectDllToOne(LPCTSTR szProc, int nMode, LPCTSTR szDllPath)
{
    ...
    if( i == nLen )     // PID , break 당하면 ProcName임
    {
...
int _tmain(int argc, TCHAR *argv[])
{
	...
	if( (argc != 2) || 
        ( _tcsicmp(argv[1], L"-i") && _tcsicmp(argv[1], L"-e")) )
	{
	    _tprintf(L"\n %s (Ver 1.0) - Dll Injection/Ejection Utility!!!\n"\
                 L"   YAK_Project\n"\
                 L"   noname@gmail.com\n"\
                 L"\n USAGE  : %s <-i|-e> \n\n", 
                 argv[0], argv[0]);
    ...
    if( !GetFullPathName(L"MalDetecter.dll", BUFSIZE, szPath, NULL) )    
    {
    ...
    if( !_tcsicmp(argv[1], L"-e") )
        ...
        InjectDllToOne(L"explorer.exe", nMode, szPath);
