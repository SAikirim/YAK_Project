...
def isSectionName(binary):
    ...
    try: 
        for section in binary.sections:
            if section.name.lower() in section_name_list:
                return False
            else:
                pass
            return True
...
def isTimeDate(binary):
    ...
    if (date2 > ts2) or (date2 < ts2 - 631152000):
        return True
    else:
        return False
...
def isDllCha(binary):
    try:
        ...
        for dll in dll_list:
            if str(dll) == 'DLL_CHARACTERISTICS.WDM_DRIVER':
                return True
            else:
                continue
        return False
...
def isPacking(binary):
    try:
        for section in binary.sections:
            ...
            break
        if (raw_size == 0 or ((virtual_size -raw_size)> (raw_size * 2)) ):
            return True
        else:
            return False
    except:
...
# IP, URL check
def isValidURL(data):
    urls = ['www.', 'http://', 'https://', '.com', '.net', '.kr', '.org', '.io']    
    try:
        for string in data:    
            string = string.decode('utf-8')
            for url in urls:
                if url in string :
                    if string == 'www.sysinternals.com':
                        continue
                    return True
                else:
                    continue
        ...
        for address in data:
            ...
            for item in parts:
                if not item.isdigit():
                    i = 1
                    break
                if not (0 <= int(item) and int(item) <= 255):
                    i = 1
                    break
            if i == 0:
                return True
...
# exAPI check
def exApi(binary):
    exApi_list=['DllCanUnloadNow','DllGetClassObject','DllRegisterServer','DllUnregisterServer']
    check_exApi=[1 for i in range(len(exApi_list))]
    try:
        for function in binary.exported_functions:
            if function.name in exApi_list:
                check_index = exApi_list.index(function.name)
                check_exApi[check_index] = 0
            else:
                continue
        return check_exApi
    except:
...
def isDll(binary):
    ...
    try:
        for dll in binary.libraries:
            if dll.upper() in mal_dll_list:
                check_dll_index = mal_dll_list.index(dll)
                check_dll[check_dll_index] = 1
        return check_dll
    except:
        return [1 for i in range(len(mal_dll_list))]
...
def isApi(binary, mal_api_list):
    ...
    try:
        for function in binary.imported_functions:
            if function.name in mal_api_list:
                check_index = mal_api_list.index(function.name)
...
if __name__ == "__main__":
    ...
    r"C:\Windows\System32\calc.exe",    #1 X    #0
    r"C:\Windows\System32\notepad.exe", #1 X    #0
    r"C:\Program Files\Internet Explorer\iexplore.exe", #0  #0
    'C:/Users/user/source/repos/Yak_project/infected1.vir', #1  #1
    'C:/Users/user/source/repos/Yak_project/infected2.vir', #1  #0 X
    'C:/Users/user/source/repos/Yak_project/infected3.vir'] #1  #1
    # 서버: OX
    for i in path:
        print(All_Check(i))
